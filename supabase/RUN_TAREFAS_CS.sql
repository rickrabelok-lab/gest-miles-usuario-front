-- =============================================================================
-- Tarefas do CS (CS Task Engine)
--
-- FAÇA ASSIM NO Supabase:
-- 1) Abra: https://supabase.com/dashboard → selecione SEU projeto
-- 2) Menu: SQL Editor → New query
-- 3) Cole TODO o conteúdo deste arquivo (Ctrl+A/Ctrl+C no Cursor) → Run
--
-- Pré-requisitos:
-- - `public.alertas_sistema` e `public.alertas_sistema_sync()` já devem existir
--   (rode antes o seu `RUN_GESTOR_SCORES_E_ALERTAS.sql` ou `npx supabase db push`).
--
-- Depois de rodar:
-- - A trigger vai criar tarefas para alertas novos/ativados automaticamente.
-- - Para tarefas de alertas ativos que já existiam antes, use no app a tela `/cs/tarefas` →
--   botão “Atualizar tarefas” (RPC: `tarefas_cs_sync_from_alertas`).
-- =============================================================================
