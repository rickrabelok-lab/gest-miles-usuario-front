...
begin
  ...
  -- Funcao tipicamente disparada por triggers de tabelas protegidas.
  -- Guard (evita chamado manual para clientes que o usuario nao gerencia).
  ...
  loop
    ...
    -- Ultimos indicadores por (cliente,gestor)
    select n.nota
    into v_nps
    ...
    -- Atividade (maior data entre saldo/movimentos e ultimas avaliacoes do gestor)
    select greatest(
...
begin
  ...
  -- Check de permissao (security definer)
  if not public.is_legacy_platform_admin() then
    if not (
  ...
  -- Evita duplicar tarefa mesmo com alertas resolvidos.
  if exists (
    select 1
  ...
  -- Mapeamento para tipos de alerta que geram tarefas_cs via tarefas_cs_create_from_alerta
  v_alert_tipo := null;
  ...
  -- Garante criacao da tarefa mesmo se o trigger nao estiver disparando por alguma razao.
  select a.id
  into v_alert_id
  ...
  -- Resolve imediatamente para nao poluir a aba de alertas ativos.
...
-- Saldo/updated_at no programa do cliente
do $$
begin
...
-- Movimentos (geram atividade / economia / engajamento)
do $$
begin
