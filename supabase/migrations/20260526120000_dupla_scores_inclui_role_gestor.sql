-- Inclui role 'gestor' no gestor_map (antes era só cs / admin_equipe).
-- Os perfis das duplas (Ana, Silmara, Tiago, Filipe, Guilherme, Jessica, Carla, Wesley, Diogo)
-- estão com role = 'gestor', e por isso a RPC anterior gerava 0 clientes para quase todas.
-- Inclui também a união cliente_gestores + gestor_clientes + equipe_clientes (nac/intl)
-- como em useGestor.ts. Ativo = não inativo (igual aos KPIs do app).
