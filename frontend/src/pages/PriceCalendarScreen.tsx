...
const PriceCalendarScreen = () => {
  ...
  return (
    <div className="mx-auto min-h-screen max-w-[480px] bg-[#F5F7F9]">
      <header className="fixed inset-x-0 top-0 z-40 flex justify-center bg-[#F5F7F9]/95 backdrop-blur">
        <div className="w-full max-w-[480px] px-4 pb-3 pt-4">
          <div className="grid grid-cols-[40px_1fr_40px] items-center">
            <button
              type="button"
              onClick={() => navigate(-1)}
              className="inline-flex h-10 w-10 items-center justify-center rounded-full bg-white text-slate-700 shadow-[0_2px_10px_rgba(15,23,42,0.05)]"
            >
              <ArrowLeft size={18} />
            </button>
            <h1 className="text-center text-[20px] font-semibold tracking-tight text-[#1E293B]">
              Seleção de voos
            </h1>
            <div />
          </div>
          <div className="mt-3 grid grid-cols-[1fr_auto] items-center gap-2">
            <div className="min-w-0">
              <p className="truncate text-[17px] font-medium text-[#0F172A]">{routeLabel}</p>
              <div className="mt-1 inline-flex items-center gap-1.5 rounded-full bg-white px-2 py-1 shadow-[0_2px_10px_rgba(15,23,42,0.05)]">
            ...
            <div className="rounded-xl bg-white px-3 py-1.5 text-right shadow-[0_2px_10px_rgba(15,23,42,0.05)]">
              <p className="text-[11px] text-[#94A3B8]">Histórico de preços</p>
      ...
      <main className="px-4 pb-28 pt-[132px]">
        <div className="mb-5 flex items-center justify-between">
          ...
          <h2 className="text-[18px] font-semibold text-[#1E293B]">{monthLabel}</h2>
          <button
            type="button"
            onClick={() => {
              setSelectedDay(null);
      ...
      <div className="fixed inset-x-0 bottom-0 z-40 flex justify-center bg-gradient-to-t from-[#F5F7F9] via-[#F5F7F9] to-transparent pt-6">
        <div className="w-full max-w-[480px] px-4 pb-5">
          <Button
            className="h-14 w-full rounded-full bg-[linear-gradient(135deg,#0EA5A4,#14B8A6)] text-base font-semibold text-white shadow-[0_6px_16px_rgba(14,165,164,0.28)] transition-all duration-150 active:scale-[0.98]"
            disabled={!canSearch}
            onClick={() => {
